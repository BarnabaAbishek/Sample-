...
async def store_file_data(client, file_data):
    """Store file data in the storage channel and return the unique ID"""
    unique_id = generate_unique_id()
    try:
        # Store the complete file data in the channel
...
async def get_file_data(client, unique_id):
    ...
    try:
        async for message in client.search_messages(STORAGE_CHANNEL_ID, query=f"FileID:{unique_id}"):
